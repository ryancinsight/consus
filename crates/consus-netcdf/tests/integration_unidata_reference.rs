...
fn test_unidata_reference_netcdf4_4_0() {
    ...
    assert!(
        !dims.is_empty(),
        "NetCDF-4 reference file should have dimensions"
    );

    let vars = &root.variables;
    assert!(
        !vars.is_empty(),
        "NetCDF-4 reference file should have variables"
    );
    ...
    for var in vars {
        assert!(
            var.rank() == var.dimensions.len(),
            "Variable rank should match dimension list"
        );

        if var.name == "lat" || var.name == "lon" || var.name == "time" {
            assert!(
                var.coordinate_variable,
                "Dimension variable should be correctly identified as coordinate variable"
            );
        }
    }
    ...
    if let Some(var) = vars.first() {
        ...
        let hdf5_mapping =
            consus_netcdf::model::bridge::map_variable(var).expect("Valid HDF5 mapping");
