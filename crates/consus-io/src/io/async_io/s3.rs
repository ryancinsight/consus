...
impl AsyncReadAt for S3Reader {
    async fn read_at(&self, pos: u64, buf: &mut [u8]) -> Result<()> {
        ...
        let response = self
            ...
            .get_object(req)
            .await
            .map_err(|e| Error::Io(std::io::Error::new(std::io::ErrorKind::Other, format!("S3 get_object failed: {}", e))))?;
...
impl AsyncLength for S3Reader {
    async fn len(&self) -> Result<u64> {
        ...
        let response = self
            ...
            .head_object(req)
            .await
            .map_err(|e| Error::Io(std::io::Error::new(std::io::ErrorKind::Other, format!("S3 head_object failed: {}", e))))?;
