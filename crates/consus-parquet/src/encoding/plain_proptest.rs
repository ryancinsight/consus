...
use super::plain::{
    decode_plain, decode_plain_byte_array, decode_plain_fixed_byte_array, decode_plain_i96,
};
...
proptest! {
    /// ∀ v ∈ i32: decode_plain::<i32>(v.to_le_bytes(), 1) == [v]
    #[test]
    fn prop_i32_plain_roundtrip(v in i32::MIN..=i32::MAX) {
        let encoded = v.to_le_bytes();
        let decoded = decode_plain::<i32>(&encoded, 1).unwrap();
...
proptest! {
    /// ∀ v ∈ i64: decode_plain::<i64>(v.to_le_bytes(), 1) == [v]
    #[test]
    fn prop_i64_plain_roundtrip(v in i64::MIN..=i64::MAX) {
        let encoded = v.to_le_bytes();
        let decoded = decode_plain::<i64>(&encoded, 1).unwrap();
        prop_assert_eq!(decoded, alloc::vec![v]);
    }
}
...
proptest! {
    /// ∀ bits ∈ u32: decode_plain::<f32>(bits_as_le_bytes, 1) == [f32::from_bits(bits)]
    ///
    ...
    fn prop_f32_plain_roundtrip_bits(bits in u32::MIN..=u32::MAX) {
        ...
        let encoded = v.to_le_bytes();
        let decoded = decode_plain::<f32>(&encoded, 1).unwrap();
...
proptest! {
    /// ∀ bits ∈ u64: decode_plain::<f64>(bits_as_le_bytes, 1) == [f64::from_bits(bits)]
    #[test]
    fn prop_f64_plain_roundtrip_bits(bits in u64::MIN..=u64::MAX) {
        let v = f64::from_bits(bits);
        let encoded = v.to_le_bytes();
        let decoded = decode_plain::<f64>(&encoded, 1).unwrap();
        prop_assert_eq!(decoded[0].to_bits(), v.to_bits(),
