...
fn decode_column_metadata(r: &mut ThriftReader<'_>) -> Result<ColumnMetadata> {
    ...
    Ok(ColumnMetadata {
        ...
        total_uncompressed_size: tus
            .ok_or_else(|| missing("parquet: ColumnMetadata missing total_uncompressed_size"))?,
        total_compressed_size: tcs
            .ok_or_else(|| missing("parquet: ColumnMetadata missing total_compressed_size"))?,
        data_page_offset: dpo
            .ok_or_else(|| missing("parquet: ColumnMetadata missing data_page_offset"))?,
...
fn decode_column_chunk(r: &mut ThriftReader<'_>) -> Result<ColumnChunkMetadata> {
    ...
    Ok(ColumnChunkMetadata {
        file_path: fp,
        file_offset: fo
            .ok_or_else(|| missing("parquet: ColumnChunkMetadata missing file_offset"))?,
        meta_data: md,
    })
}
...
fn decode_row_group(r: &mut ThriftReader<'_>) -> Result<RowGroupMetadata> {
    ...
    Ok(RowGroupMetadata {
        ...
        total_byte_size: tbs
            .ok_or_else(|| missing("parquet: RowGroupMetadata missing total_byte_size"))?,
...
mod tests {
    ...
    fn decode_file_metadata_minimal() {
        let file_bytes: &[u8] = &[
            b'P', b'A', b'R', b'1', 0x15, 0x04, // FM field 1: version=2
            ...
            0x15, 0x02, 0x25, 0x00, 0x18, 0x01, b'x', 0x00, 0x16,
            0x0A, // FM field 3: num_rows=5
