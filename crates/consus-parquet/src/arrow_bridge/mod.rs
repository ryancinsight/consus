//! Arrow bridge descriptors for Consus Parquet interoperability.
//!
//! This module defines a descriptive bridge layer between the canonical
//! Parquet schema model and Arrow-oriented execution plans. It does not
//! depend on the Arrow crate itself.
//!
//! ## Invariants
//!
//! - Bridge metadata preserves field identity, name, repetition, and physical type.
//! - Zero-copy eligibility is explicit and derived from field structure.
//! - Schema mappings are value-semantic and comparable.
//! - Integration plans remain stable under schema projection and materialization.
//!
//! ## Architecture
//!
//! - `ArrowDataTypeHint` captures the expected Arrow-facing logical shape.
//! - `ArrowZeroCopyConstraint` describes the requirements for direct exposure.
//! - `ArrowFieldDescriptor` describes one field in the bridge.
//! - `ArrowSchemaMapping` records the aggregate datatype-to-Arrow mapping.
//! - `ArrowIntegrationPlan` selects the execution policy for the bridge.
