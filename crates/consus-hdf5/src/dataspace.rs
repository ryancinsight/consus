...
//! | (v1 only) Reserved | 5 | |
//! | Dimension sizes | 8×rank | Current dimension sizes |
//! | Max dimension sizes | 8×rank | (if flag bit 0 set; 0xFFFF...=unlimited) |

use consus_core::dimension::{Extent, Shape};
...
#[cfg(feature = "alloc")]
pub fn parse_dataspace(data: &[u8], _offset_size: u8) -> consus_core::error::Result<Shape> {
    use alloc::vec::Vec;
    use byteorder::{ByteOrder, LittleEndian};

    if data.len() < 4 {
        return Err(consus_core::error::Error::InvalidFormat {
