...
impl Hdf5Link {
    ...
    pub fn parse(data: &[u8], ctx: &ParseContext) -> Result<Self> {
        ...
        let (hard_link_address, soft_link_target, external_link) = match link_type {
            LinkType::Hard => (Some(parse_hard_link_value(remaining, ctx)?), None, None),
            LinkType::Soft => (None, Some(parse_soft_link_value(remaining)?), None),
            LinkType::External => (None, None, Some(parse_external_link_value(remaining)?)),
        };
...
fn read_name_length(buf: &[u8], size: usize) -> Result<u64> {
    match size {
        1 => Ok(u64::from(buf[0])),
        2 => Ok(u64::from(LittleEndian::read_u16(buf))),
        4 => Ok(u64::from(LittleEndian::read_u32(buf))),
...
fn parse_hard_link_value(data: &[u8], ctx: &ParseContext) -> Result<u64> {
    let size = ctx.offset_bytes();
    if data.len() < size {
        return Err(Error::InvalidFormat {
            #[cfg(feature = "alloc")]
            message: alloc::format!(
                "hard link value too short: need {size} bytes, have {}",
                data.len()
            ),
        });
    }
