...
#[derive(Default)]
struct PixelDensity {
    horizontal_resolution: Option<(u32, u32)>,
    vertical_resolution: Option<(u32, u32)>,
    resolution_unit: Option<u16>,
}

#[derive(Default)]
struct PresentationMetadata {
    primary_density: PixelDensity,
    thumbnail_density: PixelDensity,
...
impl ByteOrder {
    fn read_u16(self, bytes: &[u8]) -> Result<u16, DecodeError> {
    ...
    fn read_u32(self, bytes: &[u8]) -> Result<u32, DecodeError> {
...
pub(super) fn parse(tiff: &[u8]) -> Result<Orientation, DecodeError> {
    let order = match tiff.get(..2) {
        Some(b"II") => ByteOrder::Little,
        Some(b"MM") => ByteOrder::Big,
        _ => return Err(malformed()),
    };
    if order.read_u16(tiff.get(2..).ok_or_else(malformed)?)? != 42 {
        return Err(malformed());
    }
    let root = usize::try_from(order.read_u32(tiff.get(4..).ok_or_else(malformed)?)?)
        .map_err(|_| too_large())?;
...
fn parse_directory(context: DirectoryContext<'_>) -> Result<(), DecodeError> {
    ...
    let entries = usize::from(order.read_u16(directory)?);
    if entries == 0 {
        return Err(malformed());
    }
    ...
    for entry in entries_table.chunks_exact(12) {
        let tag = order.read_u16(entry)?;
        let field_type = order.read_u16(entry.get(2..).ok_or_else(malformed)?)?;
        let count = usize::try_from(order.read_u32(entry.get(4..).ok_or_else(malformed)?)?)
            .map_err(|_| too_large())?;
        let unit = helpers::field_width(field_type)?;
        if count == 0 {
            return Err(malformed());
        }
        ...
        if size > 4 {
            let value_offset =
                usize::try_from(order.read_u32(entry.get(8..).ok_or_else(malformed)?)?)
                    .map_err(|_| too_large())?;
    ...
    let next =
        usize::try_from(order.read_u32(directory.get(entries_end..).ok_or_else(malformed)?)?)
            .map_err(|_| too_large())?;
    if next != 0 {
        ...
        helpers::push_directory(pending, pending_kinds, pending_len, next, next_kind)?;
    }
    Ok(())
}
...
fn parse_orientation(
    ...
    let value = Orientation::try_from(order.read_u16(entry.get(8..).ok_or_else(malformed)?)?)?;
...
fn parse_pointer(context: PointerContext<'_>) -> Result<(), DecodeError> {
    ...
    let target = usize::try_from(order.read_u32(entry.get(8..).ok_or_else(malformed)?)?)
        .map_err(|_| too_large())?;
    if target == 0 {
        return Ok(());
    }
    ...
    helpers::push_directory(pending, pending_kinds, pending_len, target, target_kind)
}
...
fn parse_color_space(
    ...
    match order.read_u16(entry.get(8..).ok_or_else(malformed)?)? {
        1 => Ok(()),
...
fn parse_density(context: DensityContext<'_>) -> Result<(), DecodeError> {
    ...
    let density = helpers::density_for(metadata, kind)?;
...
fn parse_resolution_unit(
    ...
    let unit = order.read_u16(entry.get(8..).ok_or_else(malformed)?)?;
    if !matches!(unit, 1..=3) {
        return Err(malformed());
    }
...
fn parse_thumbnail(
    ...
    let value = usize::try_from(order.read_u32(entry.get(8..).ok_or_else(malformed)?)?)
        .map_err(|_| too_large())?;
...
fn read_rational(tiff: &[u8], order: ByteOrder, entry: &[u8]) -> Result<(u32, u32), DecodeError> {
    let offset = usize::try_from(order.read_u32(entry.get(8..).ok_or_else(malformed)?)?)
        .map_err(|_| too_large())?;
    let end = offset.checked_add(8).ok_or_else(too_large)?;
    let bytes = tiff.get(offset..end).ok_or_else(malformed)?;
    let numerator = order.read_u32(bytes)?;
    let denominator = order.read_u32(bytes.get(4..).ok_or_else(malformed)?)?;
    if denominator == 0 {
        return Err(malformed());
    }
